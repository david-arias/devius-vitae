...
export default function EducationForm({ action, defaultValues = {}, isEdit = false }: EducationFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input name="institution" required defaultValue={defaultValues.institution ?? ''} placeholder="Universidad Nacional" className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Título / Programa *</label>
          <input name="degree" required defaultValue={defaultValues.degree ?? ''} placeholder="Diseño Gráfico" className={inputClass} />
        </div>
      </div>

      <div>
        <label className={labelClass}>Período *</label>
        <input name="period" required defaultValue={defaultValues.period ?? ''} placeholder="2018 — 2022" className={inputClass} />
      </div>

      <div>
        <label className={labelClass}>Descripción</label>
        <textarea name="description" rows={3} defaultValue={defaultValues.description ?? ''}
          placeholder="Descripción del programa..." className={inputClass} />
      </div>

      <div>
        <label className={labelClass}>Tags (separados por coma)</label>
        <input name="tags" defaultValue={(defaultValues.tags ?? []).join(', ')}
          placeholder="Diseño, Ilustración, Branding" className={inputClass} />
      </div>
      ...
      <div className="flex items-center gap-4 pt-2">
        <button type="submit" disabled={loading}
          ...
          {loading
            ...
            : <><span className="material-symbols-outlined text-[18px]">save</span>{isEdit ? 'Guardar cambios' : 'Crear educación'}</>
          }
        </button>
        <a href="/admin/education" className="font-body-md text-body-md text-on-surface-variant hover:text-on-surface transition-colors">
          Cancelar
        </a>
      </div>
    </form>
